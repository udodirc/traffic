...
(function($){
	...
	$(document).ready(function()
	{
		...
		$("li.iceMenuLiLevel_1").hover(function() 
		{
			...
			if(typeof attr !== typeof undefined && attr !== false) 
			{
				if(attr == 'false')
				{
					...
					var ulClass = $(this).children().last().attr("class");
					ulClass += ' visible';
					$(this).children().last().addClass(ulClass);
				}
			}
		},
		function() 
		{
			...
			{
				var $this = $("li[data-hover = 'true']");
				var myClass = $this.attr("class");
				$this.removeClass(myClass).addClass("iceMenuLiLevel_1 mzr-drop parent");
				$this.attr('data-hover', 'false');
				var ulClass = $this.children().last().attr("class");
				$this.children().last().removeClass(ulClass).addClass("icesubMenu icemodules sub_level_1");
			}, 500);
		});
		
		if($(".rd-mobilemenu" ).length )
		{
			var menuHtml = '<ul class="rd-mobilemenu_ul">';
			var menu = '';
			var $this = null;
			
			$("#icemegamenu").find('.iceMenuLiLevel_1').each(function()
			{
				$this = $(this);
				
				if($this.children("ul").length) 
				{	
					menu = $.trim($this.find("a:first").text());
					menuHtml += '<li class="level_1"><a class="iceMenuTitle  rd-with-ul"><span class="icemega_title icemega_nosubtitle">' + menu + '</span>';
					menuHtml += '<span class="rd-submenu-toggle"></span></a><ul class="rd-mobilemenu_submenu" style="width: 188px; display: none;">';
					menuHtml += '<li><div style="float:left;width:188px" class="iceCols"><ul>';
					
					$this.children("ul").find('.iceCols li').each(function()
					{
						var $liElement = $(this);
						
						if($liElement.text() != '')
						{
							menuHtml += '<li id="iceMenu_336" class="iceMenuLiLevel_2"><a href="' + $.trim($liElement.find('a').attr('href')) + '" class="iceMenuTitle"><span class="icemega_title icemega_nosubtitle">' + $.trim($liElement.text()) + '</span></a></li>';
						}
					});
					
					menuHtml += '</ul></div></li></ul></li>';
				}
				else
				{
					menuHtml += '<li class="level_1"><a href="' + $.trim($this.children("a").attr('href')) + '" class=" iceMenuTitle"><span class="icemega_title icemega_nosubtitle">' + $.trim($this.text()) + '</span></a></li>'
				}
			});
			...
			{
				menuHtml += '<ul/>';
				$(".rd-mobilemenu").html(menuHtml);
			}
		}
		
		$(".iceMenuTitle").on("click",function()
		{
			$this = $(this);
			
			if($this.find('.rd-submenu-toggle'))
			{
				var myClass = $this.attr("class");
				
				if(myClass == 'iceMenuTitle rd-with-ul active')
				{
					$this.removeClass(myClass).addClass("iceMenuTitle rd-with-ul");
					$this.next("ul").css('display', 'none');
				}
				else
				{
					$this.removeClass(myClass).addClass("iceMenuTitle rd-with-ul active");
