...
class Menu extends Widget
{
     ...
     * This property will be overridden by the `template` option set in individual menu items via [[items]].
     */
    public $parentSubmenuLinkTemplate = '<a href="{url}">{label}</a>';
    /**
     * @var string the template used to render the body of a menu which is a link.
     * In this template, the token `{url}` will be replaced with the corresponding link URL;
     * while `{label}` will be replaced with the link text.
     ...
     */
    protected function renderItems($items, $submenuList = false)
    {
        $n = count($items);
        $lines = [];
        ...
        foreach ($items as $i => $item) 
        {
			...
			$submenu = (!empty($item['items'])) ? true : false;
			$menu = $this->renderItem($item, $submenu, $submenuList);
            
            if (!empty($item['items'])) {
                $submenuTemplate = ArrayHelper::getValue($item, 'submenuTemplate', $this->submenuTemplate);
                $submenuTemplate = strtr($submenuTemplate, [
					'{ui}' => ((isset($item['ui'])) ? $item['ui'] : ''),
				]);
     ...
     * @return string the rendering result
     */
    protected function renderItem($item, $submenu, $submenuList)
    {
		if($this->admin)
		{
			if(isset($item['url'])) 
		...
		else
		{
			if (isset($item['url'])) 
			{
				$template = ArrayHelper::getValue($item, 'template', ($submenuList) ? $this->submenuLinkTemplate : (($submenu) ? $this->parentSubmenuLinkTemplate: $this->linkTemplate));
